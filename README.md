...
**[打开在线演示](https://violetloveai.github.io/zhihu-live-answers/)** · [产品说明计划书](https://violetloveai.github.io/zhihu-live-answers/submission/index.html) · [计划书 PDF](https://violetloveai.github.io/zhihu-live-answers/submission/plan.pdf) · [操作演示视频](https://violetloveai.github.io/zhihu-live-answers/submission/demo.html) · [功能手册](https://violetloveai.github.io/zhihu-live-answers/manual/index.html)

这是知乎黑客松作品的交互原型。它用虚构教学样例展示贡献、追问和答案修订的完整过程；演示整理结果由浏览器内的规则生成，不代表真实模型输出或真实知乎用户观点。产品计划书说明核心思路、技术方案、知乎生态结合方式，以及已实现和仍待接入的能力。
...
点击首页“开始体验”或右上角“加入我们”，选择“和看山体验完整流程”，进入演示维护者空间。页面已准备两个议题、四份虚构个人贡献、待确认问题和样例修订。

1. 写一段经历，选择昵称、匿名或化名，核对公开预览后提交。
2. 检查看山给出的关联及依据；贡献不会直接覆盖当前答案。
3. 确认一条下一问，在应用内演示讨论中回复。
4. 审阅修订的原文、建议和证据，确认采纳后查看版本历史。
5. 退出当前体验，再选择“以经验贡献者身份体验”，查看普通参与者可用的功能。

## 在线版本的范围

所有记录只保存在当前浏览器的 `localStorage` 中。刷新页面会保留当前体验；清除网站数据会重置。不同浏览器和设备之间不共享记录，退出后重新选择身份会创建新演示空间。演示角色用于展示界面流程，不是真实账号权限。

GitHub Pages 托管的是浏览器应用，不运行数据库服务、账号服务或后台任务。在线版不连接知乎 OAuth、真实模型、真实搜索或外部发布接口；来源发现、内容关联和修订均为明确标注的规则演示。请使用虚构资料体验，不要提交敏感信息。

公开版使用系统中文字体和轻量看山静态图，首屏无需下载外部字体或大型 GIF。操作视频录制同版演示的实际交互，字幕明确标记规则演示；视频中的修订来自预置样例。

功能手册的 PDF 与网页版使用本地完整版本截图，并另列在线版本的存储及接入范围。该演示不承诺跨设备同步、多人协作或真实社区回写。
